...
Surveyor_RTD::Surveyor_RTD(uint8_t pin)
{
    this->pin = pin;
...
bool Surveyor_RTD::begin()
{
#if defined(ESP8266) || defined(ESP32)
    EEPROM.begin(1024);
#endif
    if ((EEPROM.read(this->EEPROM_offset) == magic_char) && (EEPROM.read(this->EEPROM_offset + sizeof(uint8_t)) == SURVEYOR_RTD))
    {
...
float Surveyor_RTD::read_voltage()
{
    float voltage_mV = 0;
...
float Surveyor_RTD::read_calibrated_voltage()
{
    return (read_voltage() - this->rtd.voltage_offset_mV);
}

float Surveyor_RTD::secondary_temp_correction(float vcal)
{
    // 5E-06x2 - 0.0043x - 0.2768
    return 5E-06 * vcal * vcal - 0.0043 * vcal - 0.2768;
}

float Surveyor_RTD::read_RTD_temp_C(float voltage_mV)
{
...
float Surveyor_RTD::read_RTD_temp_C()
{
    return read_RTD_temp_C(read_voltage());
}

//(0°C × 9/5) + 32 = 32°F
float Surveyor_RTD::read_RTD_temp_F()
{
    return read_RTD_temp_C() * 9.0 / 5.0 + 32.0;
}

void Surveyor_RTD::cal(float cal_temp_C)
{
